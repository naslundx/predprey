...
class Pred:
	...
	def getNeighbours(self, index):
		...
		candidates = self.checkPointJump(index, candidates)
		for next in reversed(candidates):
			if not self.checkBorder(next): candidates.remove(next)
			elif not self.checkPassable(next, self.world[next]): candidates.remove(next)
			else: pass
